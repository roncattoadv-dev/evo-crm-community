module AuthHelper
  ...
  def cookie_domain
    ...
    # TLDs de segundo nível conhecidos (ex: .com.br, .co.uk) requerem 3 partes
    second_level_tlds = %w[com.br org.br net.br gov.br edu.br co.uk org.uk net.uk co.jp]
    potential_tld = domain_parts.size >= 2 ? "#{domain_parts[-2]}.#{domain_parts[-1]}" : ''

    if second_level_tlds.include?(potential_tld) && domain_parts.size >= 3
      # Para .com.br usa as 3 últimas partes: .roncattoadvocacia.com.br
      ".#{domain_parts[-3]}.#{domain_parts[-2]}.#{domain_parts[-1]}"
    elsif domain_parts.size >= 2
