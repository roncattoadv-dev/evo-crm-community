...
class ContactInboxBuilder
  ...
  def wa_source_id
    if @contact.phone_number.present?
      # Normalize through the same rules Evolution API uses to build the JID
      # (Whatsapp::PhoneNumberNormalizer, a port of createJid.ts) instead of just
      # stripping the '+'. Contacts created via API/import/widget must land on the
      # exact source_id that will arrive on the contact_inbox lookup when the
      # customer later replies on WhatsApp — otherwise a second ContactInbox gets
      # created for the same contact and the conversation thread is lost.
      Whatsapp::PhoneNumberNormalizer.call(@contact.phone_number)
