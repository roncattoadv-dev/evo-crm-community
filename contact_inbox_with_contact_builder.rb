...
class ContactInboxWithContactBuilder
  ...
  def build_contact_with_contact_inbox
    ...
    @contact_inbox ||= reuse_existing_whatsapp_contact_inbox || create_contact_inbox
  end

  # find_contact can resolve an existing Contact whose stored phone_number/identifier
  # predates a source_id normalization change (e.g. the BR "nono digito" rule), so the
  # incoming source_id doesn't exact-match any existing ContactInbox. Without this,
  # build_contact_with_contact_inbox would create a SECOND ContactInbox for the same
  # contact+inbox, splitting the conversation thread in two. Mirrors the reuse already
  # done in perform_evolution_go_lookup, extended to every WhatsApp provider.
  def reuse_existing_whatsapp_contact_inbox
    return nil unless whatsapp_channel? && source_id.present?

    existing = @contact.contact_inboxes.find_by(inbox: inbox)
    return nil unless existing
    return nil if existing.source_id == source_id

    Rails.logger.info "Whatsapp: reusing ContactInbox #{existing.id} for contact #{@contact.id} in inbox #{inbox.id} (source_id '#{existing.source_id}' -> '#{source_id}')"
    existing.update!(source_id: source_id)
    existing
  end
