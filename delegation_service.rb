  ...
  class DelegationService
    ...
    def build_message_event
      {
        ...
        key_id: @message.source_id.to_s,
        message_content: @message.content.to_s,
        message_timestamp: @message.created_at.to_i.to_s,
    ...
    def build_metadata
      ...
      content_attrs = @message.content_attributes || {}
      {
        ...
        ctwa_clid: content_attrs.dig('ctwa_clid') || content_attrs.dig(:ctwa_clid),
        ad_source_id: content_attrs.dig('ad_source_id') || content_attrs.dig(:ad_source_id)
      }
    end
    ...
    def evolution_channel
      @evolution_channel ||= @conversation.inbox.channel
    end

    def evolution_server_url
      return '' unless evolution_channel.respond_to?(:provider_config)

      evolution_channel.provider_config&.dig('api_url').to_s
    end

    def evolution_api_key
      return '' unless evolution_channel.respond_to?(:provider_config)

      evolution_channel.provider_config&.dig('admin_token').to_s
    end
