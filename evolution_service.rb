...
class Whatsapp::Providers::EvolutionService < Whatsapp::Providers::BaseService
  ...
  def send_attachment_message(phone_number, message)
    ...
    attachments.each_with_index do |attachment, index|
      include_caption = index == 0
      result = case attachment.file_type
               when 'image', 'video', 'file'
                 send_media_message(phone_number, message, attachment, 'sendMedia', include_caption: include_caption)
               when 'audio'
  ...
  def send_media_message(phone_number, message, attachment, endpoint, include_caption: true)
    # Use direct S3 URL for media
    media_url = generate_direct_s3_url(attachment)

    # Evolution API only accepts: image, document, video, audio
    # Internal file_type 'file' must be mapped to 'document'
    mediatype = case attachment.file_type
    ...
    Rails.logger.info "[Evolution Media] Sending #{attachment.file_type} (mediatype=#{mediatype}) with direct URL: #{media_url}"
    ...
    body[:caption] = html_to_whatsapp(message.content.to_s) if include_caption
