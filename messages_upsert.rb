...
module Whatsapp::EvolutionHandlers::MessagesUpsert
  ...
  def set_individual_contact
    ...
    # Normalize via Whatsapp::PhoneNumberNormalizer (the single source of truth, a
    # port of Evolution API's createJid) instead of the old always-add-the-9th-digit
    # rule. ContactInboxBuilder#wa_source_id now normalizes contacts created via
    # API/import/widget through the same function, so both sides converge on the
    # exact same source_id. Before this fix, a mismatch here meant an API-created
    # contact's first WhatsApp reply couldn't find its ContactInbox, so a duplicate
    # one was created and the conversation thread was lost (a new conversation was
    # started instead of continuing the existing one).
    source_id = Whatsapp::PhoneNumberNormalizer.call(raw_source_id)
