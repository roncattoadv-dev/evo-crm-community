...
async def create_session_sync(
    ...
    try:
        ...
        if existing_session:
            ...
            # Returning a bare dict here fails validation against this route's
            # declared response_model (SuccessResponse[SessionWithMetadata] requires
            # data/meta), which FastAPI raises as an exception inside EvoAuthMiddleware's
            # call_next() — swallowed there and misreported to the caller as a 401.
            # success_response() returns a JSONResponse directly, bypassing response_model
            # validation, same as the error_response() calls elsewhere in this function.
            return success_response(
                data={
                    "session_id": session_id,
                    "agent_id": str(agent_id),
