...
class EvoAuthMiddleware(BaseHTTPMiddleware):
    ...
    def _extract_agent_id_from_sync_path(self, path: str) -> str:
        """Extract agent_id from /sync/{...} paths.

        Two distinct shapes share this prefix:
          - POST /api/v1/sessions/sync/{agent_id}            (create_session_sync: bare agent UUID)
          - DELETE/events on /sync/{session_id}               (session_id format: {display_id}_{agent_id})

        A bare UUID has no underscore, so the composite-format split below
        never matches it and extraction silently returned None here — which
        made create_session_sync fall through to generic token auth and
        reject a perfectly valid Agent Bot API key with a 401. Try the
        composite format first, then fall back to treating the whole
        segment as the agent_id itself.
        """
        import re
        ...
        if match:
            ...
            # Fallback: bare agent_id, no composite session_id (create_session_sync)
            try:
