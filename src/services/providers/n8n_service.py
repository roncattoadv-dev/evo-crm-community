...
class N8NService:
    ...
    async def send_message(
        ...
        payload: Dict[str, Any] = {
            ...
            "messageId": context.get("messageId", "") if context else "",
            "timestamp": context.get("timestamp", "") if context else "",
            "conversationId": context.get("conversationId", "") if context else "",
            "messages": context.get("messages", []) if context else [],
        }

        # If chatInput is empty but messages have attachments, mark as media
        if not payload["chatInput"] and any(m.get("attachments") for m in payload["messages"]):
            payload["chatInput"] = "[media]"
